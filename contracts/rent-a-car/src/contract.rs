...
use crate::{interfaces::contract::RentACarContractTrait, methods::{admin::{add_car::add_car, remove_car::remove_car, set_commission::set_commission, withdraw_commission::withdraw_commission},owner::payout_owner::payout_owner,public::{get_car_status::get_car_status, initialize::initialize},renter::rental::rental,},
...
impl RentACarContractTrait for RentACarContract {
    ...
    fn set_commission(env: &Env, commission_amount: i128) -> Result<(), Error> {
        set_commission(env, commission_amount)
    }

    fn withdraw_commission(env: &Env, amount: i128) -> Result<(), Error> {
        withdraw_commission(env, amount)
    }
}
...
// use soroban_sdk::{contract, contractimpl, Address, Env};
// use crate::{events, interfaces::contract::RentACarContractTrait, methods::{public::get_car_status::get_car_status, token::token::token_transfer}, storage::{admin::{has_admin, read_admin, write_admin}, car::{has_car, read_car, remove_car, write_car}, contract_balance::{read_contract_balance, write_contract_balance}, rental::write_rental, structs::{car::Car, rental::Rental}, token::write_token, types::{car_status::CarStatus, error::Error}}};
...
//         car.available_to_withdraw = car.available_to_withdraw.checked_add(amount).ok_or
//         (Error::OverflowError)?;
