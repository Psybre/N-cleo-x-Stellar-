...
import { CONTRACT_ADDRESS, HORIZON_NETWORK_PASSPHRASE, HORIZON_URL, SOROBAN_RPC_URL, STELLAR_FRIENDBOT_URL, STELLAR_NETWORK } from "../utils/contants";
...
async payment(
    ...
    const asset = this.getAsset(
      assetCode, 
      "GCW6EG26UZTEZG2CZCZIKJVU4QDXOE3YDTIR2GKUKK4JRVQ7LIHF3G75"
    );
  ...
  async setCommission(
    ...
    const contractClient = await this.buildClient<IRentACarContract>(walletAddress);
    
    const result = await contractClient.set_commission({
    ...
    return await this.submitTransaction(signedTx.signedTxXdr);
  }
  ...
  async withdrawCommission(
    ...
    const result = await contractClient.withdraw_commission({
      amount: amount * ONE_XLM_IN_STROOPS,
    });
